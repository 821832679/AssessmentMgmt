...
public class Score extends BaseScore<Score> {
	...
	public Page<Score> getScoreNotNullPage(Integer curr, Integer pagesize, String Searcht, String orgid, String orgids, String userid) {
		if (StrKit.isBlank(Searcht) && StrKit.isBlank(orgid)) {
			return Score.dao.paginate(curr, pagesize, "select s.*,o.name orgname ", " from t_score s left join sys_user u on s.userid=u.id left join sys_org o on u.orgid=o.id where o.id in ("+orgids+") and s.userid = '"+userid+"' order by s.score desc,s.createtime desc");
		}else{
			String where = "where o.id in ("+orgids+") and s.userid = '"+userid+"'";
			if(StrKit.notBlank(Searcht))
