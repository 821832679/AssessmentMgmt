...
public class TopicType extends BaseTopicType<TopicType> {
	...
	public Page<TopicType> getTopicByStatusPage(Integer curr, Integer pagesize, String status) {
		return TopicType.dao.paginate(curr, pagesize, "select id id,name name,status status",
				" from t_topic_type where 1=1 "+ (StringUtil.isNotBlank(status)?" and status="+status:"")+ " order by id desc");
	}
	...
	public List<TopicType> getTopicByStatus(String status) {
		return TopicType.dao.find("select id id,name name,status status from t_topic_type where 1=1 "+ (StringUtil.isNotBlank(status)?" and status="+status:""));
	}
